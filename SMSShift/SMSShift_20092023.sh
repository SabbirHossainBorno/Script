...
function check_rsync_installed() {
    if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "command -v rsync >/dev/null 2>&1"; then
        ...
        echo "Rsync not found on the destination server."
        return 1
    fi
}
...
function install_rsync() {
    if ! ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "command -v rsync >/dev/null 2>&1"; then
        echo "Rsync not found on the destination server. Installing rsync..."
        ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "yum install -y rsync"
        if [ $? -eq 0 ]; then
            echo "Rsync installed successfully on the destination server."
            return 0
        else
            echo "Error: Failed to install rsync on the destination server. File transfer aborted."
            return 1
        fi
    else
        echo "Rsync found on the destination server."
...
function check_jdk_installed() {
    if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "[ -d '/usr/jdk1.8.0_111' ]"; then
        echo "JDK 'jdk1.8.0_111' found on the destination server."
        return 0
    else
        echo "Error: JDK 'jdk1.8.0_111' not found on the destination server. Please install JDK and retry."
        return 1
    fi
}
...
function check_mysql_version() {
    ...
    if [[ $mysql_version == *" 8."* ]]; then
        ...
        echo "Error: MySQL version 8 not found on the destination server."
        return 1
    fi
}
...
function check_tomcat_installed() {
    if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "[ -d '/usr/local/apache-tomcat-7.0.59' ]"; then
        echo "Tomcat directory '/usr/local/apache-tomcat-7.0.59' found on the destination server."
        return 0
    else
        echo "Tomcat directory '/usr/local/apache-tomcat-7.0.59' not found on the destination server."
        return 1
    fi
}
...
function backup_tomcat() {
    ...
    if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "[ -d '/usr/local/apache-tomcat-7.0.59' ]"; then
        ...
        if [ $? -eq 0 ]; then
            echo "Renamed existing Tomcat directory to: $backup_folder"
            return 0
        else
            echo "Error: Failed to rename the existing Tomcat directory."
            return 1
        fi
    else
        echo "Tomcat directory not found on the destination server. Skipping backup."
        return 0
    fi
}
...
function check_mongo_status() {
    if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "command -v mongod > /dev/null" && \
       ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "pgrep -x mongod > /dev/null"; then
        echo "MongoDB is installed and running on the destination server."
        return 0
    else
        echo "Error: MongoDB is not installed or not running on the destination server. Trying to restart MongoDB..."
        ...
        if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "pgrep -x mongod > /dev/null"; then
            echo "MongoDB has been restarted and is now running on the destination server."
            return 0
        else
            echo "Error: MongoDB could not be started on the destination server. Please check MongoDB installation and configuration."
            return 1
        fi
    fi
}
...
function fn_TransferFile_new_server() {
    ...
    check_tomcat_installed && backup_tomcat || echo "Skipping Tomcat backup."
...
function create_databases() {
    ...
    if [ $? -eq 0 ]; then
        echo "Databases $smsiTel_db_name and $smsfailed_db_name created successfully on the destination server."
    else
        echo "Error: Failed to create databases on the destination server."
        return 1
    fi
}
...
function run_source_files() {
    ...
    if ! ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "[ -f $smsiTel_db_sql ] && [ -f $smsfailed_db_sql ]"; then
        echo "Error: Source SQL file(s) not found on the destination server."
        return 1
    fi

    # Run the source files to create the databases on the destination server
    echo "Running The Source SQL files on the destination server($smsiTel_db_name)"
    ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "mysql -u root $smsiTel_db_name < $smsiTel_db_sql"
    if [ $? -eq 0 ]; then
        echo "Source files executed successfully for $smsiTel_db_name on the destination server."
    else
        echo "Error: Failed to execute source files for $smsiTel_db_name on the destination server."
        return 1
    fi

    echo "Running The Source SQL files on the destination server($smsfailed_db_name)"
    ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "mysql -u root $smsfailed_db_name < $smsfailed_db_sql"
    if [ $? -eq 0 ]; then
        echo "Source files executed successfully for $smsfailed_db_name on the destination server."
    else
        echo "Error: Failed to execute source files for $smsfailed_db_name on the destination server."
        return 1
    fi
}
...
function ip_change() {
    ...
    echo "Replaced IP address in Configuration.properties on the destination server."
    ...
    echo "Replaced IP address in server.cfg on the destination server."
    ...
    echo "Replaced IP address in server.xml on the destination server."
    ...
    echo "Changed IP address in MySQL database $smsiTel_db_name (vbSMSServerInfo table columns smsServerIP and smsSenderListenIP) to $destination_server."
...
function dump_iTelSMS_DB() {
    ...
    if [ $? -eq 0 ]; then
        echo
        echo -e "${BYellow}Database${Color_Off} ${BRed}$smsiTel_db_name${Color_Off} ${BYellow}Dump${Color_Off} ${BGreen}Complete.${Color_Off}"
        echo
...
function dump_iTelSMSFailed_DB() {
    ...
    if [ $? -eq 0 ]; then
        ...
        echo -e "${BCyan}$smsfailed_db_name.sql File :${Color_Off} ${BRed}$iTelSMSFAILEDSQLFile${Color_Off}"
        echo
    else
        echo
...
function collect_WholeSaleSMSServer_iTelSMSAppServer() {
    ...
    echo -e "${BCyan}Removing Old Logs [WholeSaleSMSServer$smsFolder]${Color_Off}"
    echo
    ...
    echo
    echo -e "${BCyan}Removing Old Logs [iTelSMSAppServer$smsAppFolder]${Color_Off}"
    echo
    ...
    echo
    echo -e "${BCyan}Moving [${Color_Off}${BRed}WholeSaleSMSServer$smsFolder${Color_Off} ${BCyan}&${Color_Off} ${BRed}iTelSMSAppServer$smsAppFolder${Color_Off}${BCyan}] \nTo${Color_Off}${BGreen} /usr/local/$folder_name${Color_Off}"
    echo
...
function restore_mongodb_databases() {
    ...
    if ! ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "[ -d $dump_file_SMSCampaigns ] && [ -d $dump_file_SMSContacts ]"; then
        echo "Error: MongoDB dump files not found on the destination server."
        return 1
    fi

    # Restore the SMSCampaigns MongoDB database
    echo "Restoring SMSCampaigns MongoDB database on the destination server"
    #ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "mongorestore --db $SMSCampaigns_mongodb_name $dump_file_SMSCampaigns"
    ...
    if [ $? -eq 0 ]; then
        echo "Restoration of SMSCampaigns MongoDB database was successful."
    else
        echo "Error: Failed to restore SMSCampaigns MongoDB database."
        return 1
    fi

    # Restore the SMSContacts MongoDB database
    echo "Restoring SMSContacts MongoDB database on the destination server"
    #ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "mongorestore --db $SMSContacts_mongodb_name $dump_file_SMSContacts"
    ...
    # Check the exit code of the previous command
    if [ $? -eq 0 ]; then
        echo "Restoration of SMSContacts MongoDB database was successful."
    else
        echo "Error: Failed to restore SMSContacts MongoDB database."
        return 1
    fi

    # If you reach this point, both MongoDB databases were restored successfully
    echo "MongoDB databases restored successfully on the destination server."
}
    ...
    for keyword in "${keywords[@]}"; do
        for folder in $folders; do
            if [[ "$folder" == *"$keyword"* ]]; then
                ...
                echo "Renamed /usr/local/$folder to /usr/local/${folder}_BK_$backup_timestamp on the destination server."
            fi
        done
    done

    if ! $found_folders; then
        echo "No folders with specified keywords found on the destination server."
    fi
}
...
function realocate_data() {
    ...
    for folder_name_to_move in "${folders_to_move[@]}"; do
        ...
        echo "Moved /usr/local/$folder_name/$folder_name_to_move to /usr/local/"
    done

    # Prompt for confirmation before removing the /usr/local/$folder_name directory
    read -p "Do you want to remove /usr/local/$folder_name? (yes/no): " confirmation
    if [ "$confirmation" = "yes" ] || [ "$confirmation" = "y" ] || [ "$confirmation" = "YES" ] || [ "$confirmation" = "Y" ]; then
        ...
        echo "Removed /usr/local/$folder_name"
    else
