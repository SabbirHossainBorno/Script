...
start_services() {
    ...
    itel_sms_server_log="/usr/local/WholeSaleSMSServer$smsFolder/iTelSMSServer.log"
    ...
    # Check if the log files exist on the destination server
    if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "[ -f '$itel_sms_server_log' ] && [ -f '$itel_sms_app_log' ]"; then
        
        timestamp=$(date +"%Y%m%d%H%M%S")

        # Backup iTelSMSServer.log with timestamp
        itel_sms_server_backup="${itel_sms_server_log}.${timestamp}"
        ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "mv '$itel_sms_server_log' '$itel_sms_server_backup'"

        # Backup iTelSMSApp.log with timestamp
        itel_sms_app_backup="${itel_sms_app_log}.${timestamp}"
        ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "mv '$itel_sms_app_log' '$itel_sms_app_backup'"
    ...
    # Start WholeSaleSMSServer (modify this command according to your actual service start command)
    ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "nohup sh /usr/local/WholeSaleSMSServer$smsFolder/runSMS_Server.sh > /dev/null 2>&1 &"

    # Check if WholeSaleSMSServer is running (modify this according to your service checking logic)
    sleep 5  # Wait for a few seconds before checking
    if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "pgrep -f 'WholeSaleSMSServer$smsFolder'"; then
        echo -e "${BIRed}WholeSaleSMSServer$smsFolder${Color_Off} ${BIGreen}Started Successfully${Color_Off}"
        echo
    else
        echo -e "${BIBlue}Error :${Color_Off} ${BIGreen}Failed To Start${Color_Off} ${BIYellow}WholeSaleSMSServer$smsFolder${Color_Off}"
        echo
    fi
    ...
    ps aux | grep -E '$smsFolder|$smsAppFolder|apache-tomcat-7.0.59|DiskSpaceChecker' --color=auto
...
timeout_duration=60  # Timeout in seconds
        ...
        1)
                ...
                if [[ "$run_services_response" =~ ^[Yy][Ee]?[Ss]?$ ]]; then
                    # Call the start_services function if the user's input matches 'y', 'Y', 'yes', or 'YES'
                    start_services
                else
