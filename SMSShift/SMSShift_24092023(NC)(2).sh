...
function fn_Connect() {
    ...
    if [ $? -eq 0 ]; then
        conSuccssfl=1
        echo -e "${BICyan}Connection Establishment${Color_Off} ${BIGreen}Successful${Color_Off}"
        echo
        ...
        echo
        echo -e "${BIBlue}Error :${Color_Off} ${BICyan}Connection Establishment Is${Color_Off} ${BIRed}Not Successful${Color_Off}"
        echo
        exit
    fi
}
...
function fn_TransferFile_new_server() {
    ...
    check_rsync_installed || install_rsync || return 1
...
restart_apacheTomcat() {
    # Define the path to Apache-Tomcat on the destination server
    apache_tomcat_path="/usr/local/apache-tomcat-7.0.59"

    # Full paths to commands
    pgrep_cmd="/usr/bin/pgrep"
    pkill_cmd="/usr/bin/pkill"
    shutdown_cmd="$apache_tomcat_path/bin/shutdown.sh"
    startup_cmd="$apache_tomcat_path/bin/startup.sh"

    # SSH into the destination server
    ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "cd $apache_tomcat_path/bin"

    # Check if Apache-Tomcat is running using pgrep
    if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "$pgrep_cmd -f 'apache-tomcat-7.0.59' > /dev/null"; then
        echo -e "${BICyan}Stopping${Color_Off} ${BIRed}[Apache - Tomcat]${Color_Off}"
        echo
        ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "$shutdown_cmd"
        sleep 5

        # Check if Apache-Tomcat is still running after attempted shutdown
        if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "$pgrep_cmd -f 'apache-tomcat-7.0.59' > /dev/null"; then
            echo -e "${BIYellow}[Apache - Tomcat]${Color_Off} ${BIRed}Did Not Stop{Color_Off} ${BICyan}Properly${Color_Off} \n${BIRed}Killing The Process${Color_Off}"
            echo
            ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "$pkill_cmd -f 'apache-tomcat-7.0.59'"
            sleep 5
        fi
    fi

    # Check if there are any remaining Apache-Tomcat processes using pgrep
    if ! ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "$pgrep_cmd -f 'apache-tomcat-7.0.59' > /dev/null"; then
        echo -e "${BIYellow}[Apache - Tomcat]${Color_Off} ${BIGreen}Stopped Successfully${Color_Off}"
        echo
    else
        echo -e "${BIBlue}Error :${Color_Off} ${BIRed}Failed${Color_Off} ${BICyan}To Stop${Color_Off} ${BIYellow}[Apache - Tomcat]${Color_Off}"
        echo
        # You may want to add error handling here
        return 1
    fi

    # Remove the Catalina directory
    echo -e "${BIRed}Deleting${Color_Off} ${BIYellow}Catalina${Color_Off}"
    echo
    ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "rm -rf $apache_tomcat_path/work/Catalina"

    # Remove the catalina.out log file
    echo -e "${BIRed}Deleting${Color_Off} ${BIYellow}[catalina.out]${Color_Off} ${BICyan}File${Color_Off}"
    echo
    ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "rm -f $apache_tomcat_path/logs/catalina.out"

    # Start Apache - Tomcat
    echo -e "${BGreen}Starting${Color_Off} ${BIRed}[Apache - Tomcat]${Color_Off}"
    echo
    ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "$startup_cmd"

    # Wait for Apache - Tomcat to start
    echo -e "${BICyan}Waiting For Server${Color_Off} ${BIGreen}Startup${Color_Off}"
    echo
    while ! ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "grep -q 'Server startup in' $apache_tomcat_path/logs/catalina.out"; do
        sleep 1
    done

    echo -e "${BIRed}[Apache - Tomcat]${Color_Off} ${BIGreen}Restart Successfully${Color_Off}"
    echo

    ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "ps -aux | grep apache-tomcat"
    echo
...
restart_diskSpaceChecker() {
    ...
    echo
    nohup ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "sh /usr/local/DiskSpaceChecker/runDiskSpaceChecker.sh >/dev/null 2>&1 &"

    # Check if DiskSpaceChecker is running on the destination server
    if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "pgrep -f 'DiskSpaceChecker'"; then
        echo -e "${BIYellow}DiskSpaceChecker${Color_Off} ${BIGreen}Started Successfully${Color_Off}"
        echo
    else
        echo -e "${BIBlue}Error :${Color_Off} ${BIYellow}DiskSpaceChecker${Color_Off} ${BIRed}Failed To Start${Color_Off}"
        echo
        return 1
    fi

    echo
    ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "ps -aux|grep DiskSpaceChecker"
    echo
}
...
start_iTelSMSAppServer() {
    ...
    # Check if iTelSMSAppServer is running (modify this according to your service checking logic)
    sleep 5  # Wait for a few seconds before checking
    if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "pgrep -f 'iTelSMSAppServer$smsAppFolder'"; then
...
start_WholeSaleSMSServer() {
    ...
    nohup ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "\
        sh '/usr/local/WholeSaleSMSServer$smsFolder/runSMS_Server.sh' >> /dev/null 2>&1 &" &
    ...
    while true; do
        new_lines=$(ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "\
            tail -n +'$start_line' '$existing_log_file'")
        if echo "$new_lines" | grep -Fq "started successfully"; then
            start_time=$(echo "$new_lines" | grep -oE '^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]:[0-9]{2}' | tail -n 1)
            echo
            echo -e "${BIRed}$start_time${Color_Off}${BIYellow} WholeSaleSMSServer$smsFolder${Color_Off} ${BIGreen}Started Successfully.${Color_Off}"
            echo
            break
        fi
        sleep 1
...
start_services() {
    ...
    if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "[ -f '$itel_sms_server_log' ] || [ -f '$itel_sms_server_log_2' ] || [ -f '$itel_sms_app_log' ]"; then
        ...
        if ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "[ -f '$itel_sms_server_log' ]"; then
            ...
            existing_log_file="$itel_sms_server_log_2"
        elif ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "[ -f '$itel_sms_app_log' ]"; then
            existing_log_file="$itel_sms_app_log"
        fi

        if [ -n "$existing_log_file" ]; then
            timestamp=$(date +"%Y%m%d%H%M%S")

            # Backup the existing log file with a timestamp
            log_backup="${existing_log_file}.${timestamp}"
            ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "mv '$existing_log_file' '$log_backup'"

            # Recreate the log file with the same name
            ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "touch '$existing_log_file'"

            echo -e "${BICyan}Old Main Log File Backuped ${BIGreen}Successfully${Color_Off}"
            echo
        else
            echo -e "${BIBlue}Warning: ${Color_Off}${BICyan}Old Main Log File${Color_Off} ${BIRed}Not Found${Color_Off} ${BICyan}On The Destination Server${Color_Off}"
            echo
        fi
    else
    ...
    echo -e "${BICyan}Want To ${BIGreen}Start${Color_Off} Services In The ${BIYellow}New Server${Color_Off} ${BIRed}[$destination_server]${Color_Off}${BICyan}?${Color_Off} ${BIGreen}[${Color_Off}${BIRed}Y${Color_Off}${BIGreen}/${Color_Off}${BIRed}N${Color_Off}${BIGreen}]${Color_Off} ${BICyan}:${Color_Off} "
    echo
