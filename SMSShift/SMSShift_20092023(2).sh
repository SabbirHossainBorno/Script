...
function backup_tomcat() {
    # Define the backup file name with current date and time
    backup_file="/usr/local/apache-tomcat-7.0.59_BK_$(date +'%d%m%Y_%H%M%S').tar.gz"
    # Create a backup of the Tomcat directory
    ssh -o ControlPath=$SSHSOCKET -p $ssh_port $ssh_user@$destination_server "tar -zcvf '$backup_file' -C '/usr/local/' 'apache-tomcat-7.0.59'" >/dev/null 2>&1
    if [ $? -eq 0 ]; then
        echo "Backup completed successfully. Backup file: $backup_file"
        return 0
    else
        echo "Error: Failed to create a backup of the Tomcat directory."
        return 1
    fi
}
...
function run_source_files() {
    ...
    # Check if the source files were executed successfully
    if [ $? -eq 0 ]; then
        echo "Source files executed successfully to create databases $smsiTel_db_name and $smsfailed_db_name on the destination server."
    else
        echo "Error: Failed to execute source files to create databases on the destination server."
        return 1
    fi
}
...
function ip_change() {
    ...
    # Debug: Print paths and variables
    echo "Debug: destination_server=$destination_server folder_name=$folder_name smsAppFolder=iTelSMSAppServer$smsAppFolder smsFolder=WholeSaleSMSServer$smsFolder"
    ...
    # Debug: Print config_ip_count
    echo "Debug: config_ip_count=$config_ip_count"
    ...
    # Debug: Print server_cfg_ip_count
    echo "Debug: server_cfg_ip_count=$server_cfg_ip_count"
...
timeout_duration=60  # Timeout in seconds
        ...
        1)
                ...
                if [ -d "/home/maahi/" ]; then
                    ...
                    echo "Directory /home/maahi/ copied to /usr/local/$folder_name/"
                else
                    echo "Directory /home/maahi/ does not exist on the current server."
                fi
                echo -e "${BPurple}|-----------------------------[${Color_Off} ${BCyan}FINISH${Color_Off} ${BPurple}]----------------------------|${Color_Off}"
                echo
                #|-------------------------------------------------------------------------------------------------------------------------------------------|
